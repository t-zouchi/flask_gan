...
class Discriminator(Chain):
    ...
    def __call__(self, x, alpha=1.0):
        ...
        if self.depth > 0 and alpha < 1:
            h1 = self['b%d'%(8-self.depth)](x, True)
            x2 = F.average_pooling_2d(x, 2, 2)
            h2 = F.leaky_relu(self['b%d'%(8-self.depth+1)].fromRGB(x2))
            h = h2 * (1 - alpha) + h1 * alpha
        else:
            h = self['b%d'%(8-self.depth)](x, True)
                
        for i in range(self.depth):
            h = self['b%d'%(8-self.depth+1+i)](h)
